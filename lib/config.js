...
// The save turn is opt-in: it is a whole extra model turn, and on a 5m TTL it can
// burn the entire window the compaction was supposed to fit inside. Pass --save to
// use this text, or --save-prompt / CML_SAVE_PROMPT for your own.
const SAVE_PROMPT =
...
function fromEnv(env) {
  ...
  return {
    ...
    savePrompt: truthy(env.CML_SAVE) ? SAVE_PROMPT : (env.CML_SAVE_PROMPT || DEFAULTS.savePrompt),
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < flags.length; i++) {
    ...
    switch (f) {
      ...
      case '--save': opts.savePrompt = SAVE_PROMPT; break;
      case '--save-prompt': opts.savePrompt = next(); break;
      case '--no-save': opts.savePrompt = null; break;
...
module.exports = {
  DEFAULTS, SAVE_PROMPT, fromEnv, parseArgs, buildConfig, applyObservedTtl,
